...
def hn_att(inputs, sen_len, doc_len, keep_prob1, keep_prob2, _id='1'):
    ...
    logits = softmax_layer(outputs_doc, 2 * FLAGS.n_hidden, FLAGS.random_base, keep_prob2, FLAGS.l2_reg, FLAGS.n_class)
    return logits
...
def hn(inputs, sen_len, doc_len, keep_prob1, keep_prob2, _id='1'):
    ...
    outputs_doc = bi_dynamic_rnn(cell, hiddens_sen, FLAGS.n_hidden, doc_len, FLAGS.max_doc_len, 'doc' + _id, FLAGS.t2)
    logits = softmax_layer(outputs_doc, 2 * FLAGS.n_hidden, FLAGS.random_base, keep_prob2, FLAGS.l2_reg, FLAGS.n_class)
    return logits
...
def main(_):
    ...
    with tf.device('/gpu:0'):
        ...
        if FLAGS.method == 'ATT':
            prob_o = hn_att(inputs_o, sen_len_o, doc_len_o, keep_prob1, keep_prob2, 'o')
        else:
            prob_o = hn(inputs_o, sen_len_o, doc_len_o, keep_prob1, keep_prob2, 'o')
    with tf.device('/gpu:1'):
        ...
        if FLAGS.method == 'ATT':
            prob_r = hn_att(inputs_r, sen_len_r, doc_len_r, keep_prob1, keep_prob2, 'r')
        else:
            prob_r = hn(inputs_r, sen_len_r, doc_len_r, keep_prob1, keep_prob2, 'r')
