...
def lstm(inputs, sen_len, keep_prob1, keep_prob2, _id='1'):
    ...
    hiddens = bi_dynamic_rnn(cell, inputs, FLAGS.n_hidden, sen_len, FLAGS.max_sentence_len, _id, 'last')
    return hiddens
...
def lstm_att(inputs, sen_len, keep_prob1, keep_prob2, _id='1'):
    ...
    cell = tf.contrib.rnn.LSTMCell
    inputs = tf.nn.dropout(inputs, keep_prob=keep_prob1)
    hiddens = bi_dynamic_rnn(cell, inputs, FLAGS.n_hidden, sen_len, FLAGS.max_sentence_len, _id, 'all')

    alpha = mlp_attention_layer(hiddens, sen_len, 2 * FLAGS.n_hidden, FLAGS.l2_reg, FLAGS.random_base, _id)
...
def main(_):
    ...
    with tf.device('/gpu:0'):
        ...
        inputs_o = tf.reshape(inputs_o, [-1, FLAGS.max_sentence_len, FLAGS.embedding_dim])
        if FLAGS.method == 'ATT':
            h_o = lstm_att(inputs_o, len_o, keep_prob1, keep_prob2, 'o')
        else:
            h_o = lstm(inputs_o, len_o, keep_prob1, keep_prob2, 'o')
        prob_o = softmax_layer(h_o, 2 * FLAGS.n_hidden, FLAGS.random_base, keep_prob2, FLAGS.l2_reg, FLAGS.n_class, 'o')
    with tf.device('/gpu:1'):
        inputs_r = tf.nn.embedding_lookup(word_embedding_r, x_r)
        inputs_r = tf.reshape(inputs_r, [-1, FLAGS.max_sentence_len, FLAGS.embedding_dim])
        if FLAGS.method == 'ATT':
            h_r = lstm_att(inputs_r, len_r, keep_prob1, keep_prob2, 'r')
        else:
            h_r = lstm(inputs_r, len_r, keep_prob1, keep_prob2, 'r')
